...
# SyncProxyNet imported lazily inside __init__ when needed
...
class MotionDiffusion(nn.Module):
    def __init__(
        ...
        sync_proxy_ckpt='',
        mouth_dims=None,      # list[int] of absolute dim indices
        # intermediate ECS sync (Change 4, instantiated here for joint training)
        ...
        # ---- sync proxy loss (Change 3) ----
        self.use_sync_proxy_loss = use_sync_proxy_loss
        self.lambda_sync_proxy = lambda_sync_proxy
        self.sync_proxy_audio_dim = sync_proxy_audio_dim
        self.mouth_dims = list(mouth_dims) if mouth_dims else []
        self.sync_proxy_final = None
        if use_sync_proxy_loss and self.mouth_dims:
            from .sync_proxy import SyncProxyNet
            self.sync_proxy_final = SyncProxyNet(
                motion_in_dim=len(self.mouth_dims),
                audio_in_dim=sync_proxy_audio_dim,
                embed_dim=sync_proxy_embed_dim,
                window=5,
            )
            if sync_proxy_ckpt and os.path.isfile(sync_proxy_ckpt):
                _ckpt = torch.load(sync_proxy_ckpt, map_location='cpu')
                self.sync_proxy_final.load_state_dict(_ckpt, strict=False)
                print(f'[SyncProxy] Loaded final proxy ckpt: {sync_proxy_ckpt}')

        # ---- intermediate ECS sync (Change 4) ----
        self.use_intermediate_sync = use_intermediate_sync
        self.lambda_sync_intermediate = lambda_sync_intermediate
        self.sync_proxy_intermediate = None
        if use_intermediate_sync and self.mouth_dims:
            from .sync_proxy import SyncProxyNet
            # ECS cond_tokens have latent_dim=512; ground-truth mouth kp → motion side
            self.sync_proxy_intermediate = SyncProxyNet(
                motion_in_dim=len(self.mouth_dims),
                audio_in_dim=512,   # MotionDecoder latent_dim
                embed_dim=sync_proxy_embed_dim,
                window=5,
            )
    ...
    def p_losses(self, x_start, cond_frame, cond, t):
        ...
        # reconstruct — request intermediate cond_tokens when needed (Change 4)
        need_intermediate = (
            self.use_intermediate_sync and
            self.sync_proxy_intermediate is not None and
            self.mouth_dims
        )
        _model_out = self.model(
            x_noisy, cond_frame, cond, t,
            cond_drop_prob=self.cond_drop_prob,
            return_cond_tokens=need_intermediate,
        )
        if need_intermediate:
            x_recon, cond_tokens_ecs = _model_out   # [B,L,265], [B,L,512]
        else:
            x_recon = _model_out
            cond_tokens_ecs = None
        ...
        if self.part_w_dict:
            ...
            # ---- Change 3: final-output sync proxy loss ----
            if (self.use_sync_proxy_loss and
                    self.sync_proxy_final is not None and
                    self.mouth_dims):
                # mouth dims from predicted motion
                mouth_pred = model_out[..., self.mouth_dims]   # [B, L, mouth_dim]
                # first sync_proxy_audio_dim dims of cond = raw HuBERT features
                audio_feat = cond[..., :self.sync_proxy_audio_dim]  # [B, L, audio_dim]
                sp_loss = self.sync_proxy_final(mouth_pred, audio_feat)
                loss_dict['sync_proxy_final'] = sp_loss * self.lambda_sync_proxy

            # ---- Change 4: intermediate ECS sync loss ----
            if (self.use_intermediate_sync and
                    self.sync_proxy_intermediate is not None and
                    cond_tokens_ecs is not None and
                    self.mouth_dims):
                # ground-truth mouth kp trajectory as the "motion" side
                mouth_gt = x_start[..., self.mouth_dims]       # [B, L, mouth_dim]
                # ECS latent tokens as the "audio" side (shape [B, L, 512])
                si_loss = self.sync_proxy_intermediate(mouth_gt, cond_tokens_ecs)
                loss_dict['sync_intermediate'] = si_loss * self.lambda_sync_intermediate

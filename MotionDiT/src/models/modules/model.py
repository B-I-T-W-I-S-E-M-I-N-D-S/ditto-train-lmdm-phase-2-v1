...
class MotionDecoder(nn.Module):
    ...
    def forward(
        self, x: Tensor, cond_frame: Tensor, cond_embed: Tensor, times: Tensor,
        cond_drop_prob: float = 0.0,
        return_cond_tokens: bool = False,   # training-only; Change 4
    ):
        ...
        # Capture ECS output BEFORE CFG dropout so intermediate sync always sees
        # real audio encoding regardless of dropout (Change 4)
        cond_tokens_pre_dropout = cond_tokens  # [B, L, latent_dim]
        ...
        if return_cond_tokens:
            return output, cond_tokens_pre_dropout
        return output
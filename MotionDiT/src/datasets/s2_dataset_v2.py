...
class Stage2Dataset(Dataset):
    def __init__(
            ...
            neutralize_mouth_ref=False,    # zero-out mouth dims in kp_cond
            mouth_dims=(),                 # tuple/list of absolute dim indices
            mouth_neutral_npy="",          # path to mtn_mean_var npy; empty = use zeros
            **kwargs):
        super().__init__()
        ...
        # mouth neutralization state
        self.neutralize_mouth_ref = neutralize_mouth_ref
        self.mouth_dims = list(mouth_dims) if mouth_dims else []
        self.mouth_neutral_vals = None  # per-dim neutral value array [265]
        if neutralize_mouth_ref and self.mouth_dims:
            if mouth_neutral_npy and os.path.isfile(mouth_neutral_npy):
                # mtn_mean_var_npy has shape [2, dim]: row0=mean, row1=var
                _mv = np.load(mouth_neutral_npy)
                self.mouth_neutral_vals = _mv[0].astype(np.float32)  # use dataset mean
                print(f'[NeutralMouth] Loaded neutral values from {mouth_neutral_npy}')
            else:
                print('[NeutralMouth] No neutral npy provided — will use zeros.')
    ...
    def getitem(self, idx):
        ...
        # Neutralize mouth dims in kp_cond to break reference leakage (Change 1)
        if self.neutralize_mouth_ref and self.mouth_dims:
            kp_cond = kp_cond.copy()  # never mutate the cached array
            if self.mouth_neutral_vals is not None:
                kp_cond[self.mouth_dims] = self.mouth_neutral_vals[self.mouth_dims]
            else:
                kp_cond[self.mouth_dims] = 0.0
